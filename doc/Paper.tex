...
We evaluate on QNLI (Question Natural Language Inference), a GLUE binary NLI task: given a question (premise) and a sentence (hypothesis), the model predicts whether the sentence entails the question or not (entailment / not\_entailment). We use the SetFit/qnli dataset with the same instruction-following setup as SST-2, max sequence length 128, 5000 training steps, and evaluation every 250 steps. All experiments use 10 seeds (42--51). We report two metrics: \textbf{validation accuracy} is token-level denoising accuracy on the validation set (same metric as training); \textbf{classification-head accuracy} is the accuracy of a linear classifier on the [CLS] representation, evaluated on validation and test sets after training.

Table~\ref{tab:qnli_results} reports results. Full fine-tuning, LoRA-Diffusion, Weight LoRA, and BitFit achieve 100\% validation accuracy (mean over 10 seeds). Adapters reach 68.06\% $\pm$ 1.71\%, consistent with standard adapter tuning on this task in our diffusion setup. LoRA-Diffusion matches full fine-tuning on QNLI while using 28.7\% trainable parameters, demonstrating that trajectory-level adaptation transfers to NLI. Classification-head validation and test accuracies align with token-level validation accuracy for all methods.

\begin{table}[h]
\centering
\caption{Performance on QNLI (mean $\pm$ std over 10 seeds, seeds 42--51). Val acc.\ = token-level denoising; Cl.-head = linear classifier on [CLS]. Job 44084887.}
\label{tab:qnli_results}
\begin{tabular}{@{}lccccc@{}}
\toprule
Method & Train acc.\ (\%) & Val acc.\ (\%) & Cl.-head val.\ (\%) & Cl.-head test\ (\%) & Relative \\ \midrule
Full Fine-Tuning & $100.00 \pm 0.00$ & $100.00 \pm 0.00$ & $100.00 \pm 0.00$ & $100.00 \pm 0.00$ & 100.0\% \\
LoRA-Diffusion & $99.71 \pm 0.13$ & $100.00 \pm 0.00$ & $100.00 \pm 0.00$ & $100.00 \pm 0.00$ & 100.0\% \\
Weight LoRA & $100.00 \pm 0.00$ & $100.00 \pm 0.00$ & $100.00 \pm 0.00$ & $100.00 \pm 0.00$ & 100.0\% \\
Adapters & $66.69 \pm 1.57$ & $68.06 \pm 1.71$ & $100.00 \pm 0.00$ & $100.00 \pm 0.00$ & 68.1\% \\
BitFit & $100.00 \pm 0.00$ & $100.00 \pm 0.00$ & $100.00 \pm 0.00$ & $100.00 \pm 0.00$ & 100.0\% \\
\bottomrule
\end{tabular}
\end{table}
...
QNLI & 100.0\% & (to be filled) & (to be filled) & (to be filled) \\ \bottomrule
\end{tabular}
\end{table}
