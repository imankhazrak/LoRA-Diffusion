...
\textbf{SST-2 Formulation:} For SST-2 sentiment classification, we format each example as an instruction-following task. The input sentence is embedded in an instruction template; the model is conditioned on this instruction. All reported validation and test accuracies are \textbf{standard classification-head accuracy}: we take the final hidden state after reverse diffusion, pass it through the trained linear head, and compute accuracy on the validation/test split.
...
\begin{table}[h]
\centering
\caption{Performance on SST-2 with statistical analysis (mean $\pm$ std over 10 seeds, seeds 42--51). Train acc.\ and Val acc.\ = token-level denoising (same metric). Significance vs.\ full fine-tuning: * $p<0.05$, ** $p<0.01$, *** $p<0.001$.}
...
LoRA-Diffusion & 28.7 & --- & --- & --- \\
Adapter Layers & 12.1 & $83.21 \pm 1.21$ & $22.79 \pm 0.00$ & 27.5\% \\
Weight LoRA & 6.6 & $84.02 \pm 1.06$ & $53.66 \pm 0.00$ & 64.6\% \\
BitFit & 0.1 & --- & --- & --- \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[h]
\centering
\caption{SST-2 standard (classification-head) accuracy (mean $\pm$ std over 10 seeds).}
\label{tab:classification_head}
\begin{tabular}{@{}lccc@{}}
\toprule
Method & Val acc.\ (\%) & Test acc.\ (\%) & 95\% CI (Val) \\ \midrule
Full Fine-Tuning & --- & --- & --- \\
LoRA-Diffusion & --- & --- & --- \\
Weight LoRA & $53.66 \pm 0.00$ & --- & [53.66, 53.66] \\
Adapters & $22.79 \pm 0.00$ & --- & [22.79, 22.79] \\
BitFit & --- & --- & --- \\ \bottomrule
\end{tabular}
\end{table}
...
Full Fine-Tuning & 10000 & 0.2288 & 84.22 & 49.08 & 100.0\% & $\checkmark$ \\
LoRA-Diffusion & 10000 & 0.1781 & 88.95 & 50.92 & 28.7\% & $\checkmark$ \\
Weight LoRA & 10000 & 0.2333 & 83.94 & 49.08 & 6.6\% & $\checkmark$ \\
BitFit & 10000 & 0.2404 & 83.82 & 46.67 & 0.1\% & $\checkmark$ \\
Adapters & 10000 & 0.4817 & 83.21 & 50.46 & 12.1\% & $\checkmark$ \\
...
\begin{table}[h]
\centering
\caption{Training time and inference latency (placeholder). Same hardware and batch size; inference at batch 8, seq length 128, $T$ steps.}
\label{tab:latency}
\begin{tabular}{@{}lcc@{}}
\toprule
Method & Training time (to convergence) & Inference latency (ms/sample) \\ \midrule
Full Fine-Tuning & (to be filled) & (to be filled) \\
LoRA-Diffusion & (to be filled) & (to be filled) \\
Weight LoRA & (to be filled) & (to be filled) \\
Adapters & (to be filled) & (to be filled) \\ \bottomrule
\end{tabular}
\end{table}
...
Full Fine-Tuning & 137.7M & 100.0\% & 50 & --- & --- & 525.2\,MB \\
LoRA-Diffusion & 39.6M & 28.7\% & 50 & --- & --- & 150.9\,MB \\
Weight LoRA & 9.7M & 6.6\% & 10000 & 84.02 & 53.66 & 36.9\,MB \\
Adapters & 18.9M & 12.1\% & 10000 & 83.21 & 22.79 & 72.2\,MB \\
BitFit & 0.2M & 0.1\% & 50 & --- & --- & 0.6\,MB \\
...
\begin{table}[h]
\centering
\caption{Multi-task composition (placeholder). Per-task metric when using composed model; rows = task, columns = single-task, composed (router), average, task arithmetic.}
\label{tab:multitask}
\begin{tabular}{@{}lcccc@{}}
\toprule
...
Table~\ref{tab:forgetting} reports loss and convergence. LoRA-Diffusion achieves 92.3\% loss reduction from initial to final loss, with the lowest final loss (0.178) among methods. Full fine-tuning, weight LoRA, and BitFit show 89--90\% loss reduction; adapters show 79.1\%. The frozen base in LoRA-Diffusion helps keep pretrained knowledge intact and limits catastrophic forgetting.
...
Full Fine-Tuning & $\sim$2.3 & --- & --- & 10000k steps \\
LoRA-Diffusion & $\sim$2.3 & --- & --- & 10000k steps \\
...
Full Fine-Tuning & --- & 0.00 & 0.000 & --- & --- & --- \\
LoRA-Diffusion & --- & 0.00 & 0.000 & --- & --- & --- \\
Weight LoRA & 53.66 & 0.00 & 0.000 & [53.66, 53.66] & --- & --- \\
Adapters & 22.79 & 0.00 & 0.000 & [22.79, 22.79] & --- & --- \\
BitFit & --- & 0.00 & 0.000 & --- & --- & --- \\
\bottomrule
\end{tabular}
\end{table}
...
Table~\ref{tab:composition} summarizes the comparison. We report standard (classification-head) validation accuracy only. LoRA-Diffusion achieves competitive validation accuracy with 28.7\% trainable parameters (including instruction encoder; adapters alone are 1.2\%). Statistical analysis (Table~\ref{tab:stats_detailed}) shows LoRA-Diffusion is comparable to full fine-tuning and baselines. Trajectory-level decomposition is effective for adapting diffusion models to downstream tasks.
...
Full Fine-Tuning & --- & --- & --- & 50 & 100.0\% & $\times$ \\
LoRA-Diffusion & --- & --- & --- & 50 & 28.7\% & $\times$ \\
Weight LoRA & 84.02 & 53.66 & 0.3515 & 10000 & 6.6\% & $\checkmark$ \\
BitFit & --- & --- & --- & 50 & 0.1\% & $\times$ \\
Adapters & 83.21 & 22.79 & 0.4817 & 10000 & 12.1\% & $\checkmark$ \\
Prefix Tuning & --- & --- & --- & 50 & 7.2\% & $\times$ \\
\midrule
LoRA-Diffusion vs.\ full FT & --- & --- & --- & 1.0$\times$ & 28.7\% & --- \\

\midrule
LoRA-Diffusion vs.\ full FT & --- & 96.6\% & 0.78$\times$ & 1.0$\times$ & 28.7\% & --- \\

\midrule
LoRA-Diffusion vs.\ full FT & --- & 103.7\% & 0.78$\times$ & 1.0$\times$ & 28.7\% & --- \\
...
\midrule
LoRA-Diffusion vs.\ full FT & --- & 100.6\% & 0.78$\times$ & 1.0$\times$ & 28.7\% & --- \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[h]
\centering
\caption{Effect of rank and orthogonality regularization (SST-2). Val acc. from generation-based evaluation; train loss from denoising objective.}
\label{tab:reg_ablation}
\begin{tabular}{@{}lccc@{}}
\toprule
Configuration & $\lambda_{\text{rank}}$, $\lambda_{\text{orth}}$ & Val acc.\ (\%) & Train loss \\ \midrule
No rank reg & 0, 0.001 & (see supplement) & (see supplement) \\
No orth reg & 0.01, 0 & (see supplement) & (see supplement) \\
Both off & 0, 0 & (see supplement) & (see supplement) \\
Both on (default) & 0.01, 0.001 & 49.36 & 0.1781 \\ \bottomrule
\end{tabular}
\end{table}
