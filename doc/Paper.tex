...
Full Fine-Tuning & $83.81 \pm 0.75$ & --- & [83.35, 84.28] \\
LoRA-Diffusion & $86.28 \pm 3.77$ & --- & [83.94, 88.62] \\
Weight LoRA & $83.71 \pm 0.70$ & --- & [83.27, 84.14] \\
Adapters & $83.63 \pm 0.62$ & --- & [83.25, 84.01] \\
BitFit & $83.72 \pm 0.78$ & --- & [83.23, 84.20] \\ \bottomrule
\end{tabular}
\end{table}
...
\begin{table}[h]
\centering
\caption{Multi-task composition. Per-task validation metric (\%): classification-head accuracy for SST-2 and QNLI; F1 or accuracy for MRPC. Columns: Single-task (adapter only), Composed (router), Average ($w_j=1/M$), Task arithmetic (sum of deltas).}
\label{tab:multitask}
\begin{tabular}{@{}lcccc@{}}
\toprule
...
\begin{table}[h]
\centering
\caption{Effect of rank and orthogonality regularization (SST-2). Val acc.\ (token-level) = denoising accuracy; Val acc.\ (class.\ head) = classification-head; train loss from denoising objective. Results from job 44066468 (seed 42, 5k steps).}
\label{tab:reg_ablation}
\begin{tabular}{@{}lcccc@{}}
\toprule
Configuration & $\lambda_{\text{rank}}$, $\lambda_{\text{orth}}$ & Val acc.\ (token, \%) & Val acc.\ (class.\ head, \%) & Train loss \\ \midrule
No rank reg & 0, 0.001 & 88.4 & 68.1 & 0.1752 \\
No orth reg & 0.01, 0 & 83.3 & 48.2 & 0.2428 \\
Both off & 0, 0 & 89.3 & 69.0 & 0.1576 \\
Both on (default) & 0.01, 0.001 & 82.4 & 50.9 & 0.2432 \\ \bottomrule
\end{tabular}
\end{table}
...
\begin{table}[h]
\centering
\caption{Data efficiency: validation accuracy (\%) by training data fraction. \textbf{Token}: token-level denoising accuracy. \textbf{Class head}: classification-head validation accuracy. Results from job 44079308 (seed 42).}
